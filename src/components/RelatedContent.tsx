...
function ItemCard({ item }: { item: RelatedItem }) {
  ...
  const className =
    "block rounded-xl border border-border bg-card p-3 transition hover:border-primary";
  ...
  const flik =
    item.type === "goalkeeper" ? "malvakt" : item.type === "session" ? "pass" : "ovningar";
  return (
    <Link
      to="/ovningsbank"
      search={{ flik, markera: item.id }}
      aria-label={aria}
      className={className}
    >
      {inner}
