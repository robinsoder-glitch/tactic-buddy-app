...
export const QUALITY_PRESETS: Record<
  ExportQuality,
  { label: string; width: number; colors: number; bitrate: number }
> = {
...
export async function exportGif(options: ExportOptions, filename: string) {
  ...
  encoder.finish();
  download(
    new Blob([encoder.bytesView() as unknown as BlobPart], { type: "image/gif" }),
    `${filename}.gif`,
  );
}
...
export async function exportVideo(options: ExportOptions, filename: string) {
  ...
  const recorder = new MediaRecorder(stream, {
    mimeType,
    videoBitsPerSecond: options.bitrate ?? 4_000_000,
  });
