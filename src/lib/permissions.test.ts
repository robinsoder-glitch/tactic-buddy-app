...
describe("canRemoveLeader", () => {
  ...
  it("lagägaren kan inte tas bort", () => {
    expect(canRemoveLeader({ actor: owner, targetUserId: "u1", ownerUserId: "u1", actorUserId: "u1" })).toBe(false);
  });
  it("ledare kan inte ta bort andra ledare", () => {
    expect(canRemoveLeader({ actor: coach, targetUserId: "u2", ownerUserId: "u9", actorUserId: "u1" })).toBe(false);
  });
  it("lagägaren kan ta bort en annan ledare", () => {
    expect(canRemoveLeader({ actor: owner, targetUserId: "u2", ownerUserId: "u1", actorUserId: "u1" })).toBe(true);
  });
});
...
describe("dataminimering för spelare", () => {
  ...
  it("behåller exakt datum bara när det valts", () => {
    expect(toStoredBirth({ year: "2018", exactDate: "2018-05-04", useExact: true })).toBe("2018-05-04");
    expect(toStoredBirth({ year: "2018", exactDate: "2018-05-04", useExact: false })).toBe("2018-01-01");
  });
...
describe("friendlyError", () => {
  it("döljer databasfel", () => {
    expect(friendlyError(new Error('new row violates row-level security policy for table "teams"'))).toBe(
      "Du har inte behörighet till det här.",
    );
  });
  ...
  it("behåller våra egna svenska meddelanden", () => {
    expect(friendlyError(new Error("Ange en giltig e-postadress"))).toBe("Ange en giltig e-postadress");
  });
