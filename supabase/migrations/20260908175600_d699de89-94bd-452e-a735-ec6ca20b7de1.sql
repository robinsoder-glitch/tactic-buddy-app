-- 1. Kolumnvisa rättigheter på teams: koderna får inte läsas eller ändras direkt.
REVOKE SELECT, UPDATE ON public.teams FROM authenticated;
GRANT SELECT (id, club_id, name, age_group, gender, photo_path, about, created_by,
  created_at, updated_at, home_ground, archived_at, game_format) ON public.teams TO authenticated;
GRANT UPDATE (club_id, name, age_group, gender, photo_path, about,
  updated_at, home_ground, archived_at, game_format, created_by) ON public.teams TO authenticated;

-- 2. Vuxenkontrollen ska ske innan guardian-medlemskapet skapas.
...
-- 3. Servervalidering av födelsedatum på profilen.
...
BEGIN
  IF NEW.birth_date IS NOT NULL THEN
    IF NEW.birth_date > (now() AT TIME ZONE 'Europe/Stockholm')::date THEN
      RAISE EXCEPTION 'Födelsedatumet kan inte ligga i framtiden.';
    END IF;
    IF NEW.birth_date < date '1900-01-01' THEN
      RAISE EXCEPTION 'Ange ett giltigt födelsedatum.';
    END IF;
  END IF;
  RETURN NEW;
END;
