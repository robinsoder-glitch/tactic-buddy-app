-- 1. Passerad sista svarsdag ska inte blockera övriga ändringar ----------
...
BEGIN
  ...
  -- En redan passerad svarsdag får behållas oförändrad, så att ledaren kan
  -- byta in en ersättare eller ändra texten sent. Ett *nytt* datum måste
  -- däremot ligga framåt i tiden.
...
-- 2. Samma regler i de äldre funktionerna, plus notis vid ny kallelse -----
...
BEGIN
  ...
  SELECT max(respond_by) INTO cur_respond_by
  FROM public.event_invitations WHERE event_id = _event_id AND status <> 'revoked';
  IF _respond_by IS NOT NULL AND _respond_by < current_date
     AND _respond_by IS DISTINCT FROM cur_respond_by THEN
    RAISE EXCEPTION 'Sista svarsdag kan inte vara ett datum som redan passerat.';
  END IF;
  ...
  -- Nyckeln innehåller den här publiceringsomgången, så en spelare som kallas
  -- igen efter återkallelse alltid får en ny notis.
