DROP FUNCTION IF EXISTS public.get_team_leaders(uuid);

CREATE OR REPLACE FUNCTION public.get_team_leaders(_team_id uuid)
RETURNS TABLE(user_id uuid, display_name text, email text, phone text, is_owner boolean, role text)
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  ...
  WHERE tm.team_id = _team_id
    ...
    AND tm.role IN ('head_coach', 'coach', 'club_admin')
