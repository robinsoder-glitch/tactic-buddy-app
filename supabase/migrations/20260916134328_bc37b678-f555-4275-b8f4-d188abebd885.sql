...
BEGIN
  ...
  RETURNING id INTO left_player_ids;

  SELECT coalesce(array_agg(id), '{}') INTO left_player_ids
  FROM public.players
  WHERE team_id = _team_id AND is_active = false AND left_at IS NOT NULL
  ...
  -- Obesvarade kallelser ska inte ligga kvar och vänta på svar.
  IF array_length(left_player_ids, 1) > 0 THEN
