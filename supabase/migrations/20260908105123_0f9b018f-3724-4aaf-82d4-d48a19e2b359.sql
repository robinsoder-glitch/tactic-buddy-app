CREATE TABLE IF NOT EXISTS public.operation_results (
...
GRANT SELECT ON public.operation_results TO authenticated;
GRANT ALL ON public.operation_results TO service_role;
ALTER TABLE public.operation_results ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "Users read own operation results" ON public.operation_results;
CREATE POLICY "Users read own operation results"
  ON public.operation_results FOR SELECT TO authenticated
...
BEGIN
  ...
  -- Kvitto: samma handling som görs om ska ge samma svar, inte skriva om
  -- något som hunnit ändras efteråt.
  SELECT r.result INTO prior
  FROM public.operation_results r
  WHERE r.op_id = op AND r.scope = 'save_invitation_plan';
  ...
  IF _notify AND _updated > 0 THEN
    WITH targets AS (
      ...
      WHERE i.event_id = _event_id
        ...
        -- Nytillagda mottagare har redan fått "Ny kallelse" i samma handling
        -- och ska inte också få "Kallelsen har ändrats".
  ...
  VALUES (op, uid, 'save_invitation_plan', _event_id, payload)
